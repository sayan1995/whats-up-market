...
function up11(al,str)
{

		var reply="";
		elav=document.getElementById("addc").value;
		elav =parseInt(elav); 
		if (window.XMLHttpRequest) {
            // I_ID for IE7+, Firefox, Chrome, Opera, Safari
            xmlhttp = new XMLHttpRequest();
        ...
        xmlhttp.open("GET","./validate.php?itemId="+str+"&value="+elav,true);
        xmlhttp.send();
}
function up1(str) {
     // document.getElementById("cart").innerHTML = "s";
...
<!--	<table>
	...
	<?php endif; ?>
		...
		<div class="product-item" style="height:271px;">
			...
			<input type="button" id="b1" style="color:white;background-color:green;border-color:green;border-width:1px; "value="Add to cart" onclick="up11((addc.value),'<?php echo $temp ?>')"/>
			</div>
			</div>
			</div>
